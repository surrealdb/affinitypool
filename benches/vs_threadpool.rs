...
//! forget. To compare against `Threadpool::spawn(closure).await`,
//! each execute is wrapped in a `tokio::sync::oneshot`. That handshake
//! is the cost of using `threadpool` from async code and is part of
...
use threadpool::ThreadPool as LegacyPool;
...
fn legacy_pool(workers: usize) -> Arc<LegacyPool> {
	Arc::new(LegacyPool::new(workers))
}

fn bench_spawn_overhead(c: &mut Criterion) {
	let mut group = c.benchmark_group("vs_threadpool_spawn_overhead");
	...
	for &count in &[1usize, 100, 1_000, 10_000] {
		...
		for &workers in &[1usize, 4] {
			...
			// threadpool
			group.bench_with_input(
				BenchmarkId::new(format!("threadpool_{}w", workers), count),
				&(workers, count),
				|b, &(workers, count)| {
					let rt = affinitypool_runtime();
					let pool = legacy_pool(workers);
					b.iter_custom(|iters| {
						let pool = pool.clone();
						rt.block_on(async move {
							let mut total = Duration::ZERO;
...
fn bench_round_trip(c: &mut Criterion) {
	let mut group = c.benchmark_group("vs_threadpool_round_trip");
	group.measurement_time(Duration::from_secs(10));
	...
	for &workers in &[1usize, 4, 8] {
		...
		group.bench_with_input(BenchmarkId::new("threadpool", workers), &workers, |b, &workers| {
			let rt = affinitypool_runtime();
			let pool = legacy_pool(workers);
			b.iter_custom(|iters| {
				let pool = pool.clone();
...
fn bench_multi_producer(c: &mut Criterion) {
	let mut group = c.benchmark_group("vs_threadpool_multi_producer");
	group.measurement_time(Duration::from_secs(10));
	...
	for &producers in &[2usize, 4, 8] {
		for &workers in &[1usize, 4] {
			...
			// threadpool
			group.bench_with_input(
				BenchmarkId::new(format!("threadpool_{}p_{}w", producers, workers), total_tasks),
				&(producers, workers),
				|b, &(producers, workers)| {
					let rt = TokioBuilder::new_multi_thread()
						.worker_threads(producers)
